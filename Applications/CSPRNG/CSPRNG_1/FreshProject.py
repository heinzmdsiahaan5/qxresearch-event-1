...
def measure_noise():
    ...
    while time.time() - start_time < 5:  # measure for 5 seconds
        noise.append(secrets.SystemRandom().random())
    return noise
...
def generate_numbers():
    noise = measure_noise()
    secrets.SystemRandom().seed(sum(noise))
    random_numbers = []
    for i in range(10):  # generate 10 random numbers
        random_numbers.append(secrets.SystemRandom().random() * data["main"]["temp"] * data["wind"]["speed"])
